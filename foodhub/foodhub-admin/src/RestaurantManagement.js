...
const RestaurantManagement = () => {
  ...
  const handleCreateRestaurant = async () => {
    try {
      ...
      setAddress('');
      setCuisine('');
      // After successfully creating a restaurant, refetch the list
      fetchRestaurants();
    } catch (error) {
  ...
  const handleRestaurantClick = (restaurantId, restaurantName) => {
    navigate(`/admin/menu/${restaurantId}`, { state: { restaurantName } });
  };
        ...
        {restaurants.map((restaurant) => (
          <Grid item xs={12} sm={6} md={4} key={restaurant._id}>
            <Paper
              ...
              onClick={() => handleRestaurantClick(restaurant._id, restaurant.name)}
