...
const MenuManagement = () => {
  ...
  const location = useLocation(); // Get restaurantName from state
  const restaurantName = location.state?.restaurantName || ''; // Fallback if state is not passed
  ...
  return (
    <Container className={classes.container}>
      <Typography variant="h4" gutterBottom>
        Manage Menu for {restaurantName}
      </Typography>
      <Paper className={classes.paper}>
        <Typography variant="h6">Add a New Menu Item to {restaurantName}</Typography>
